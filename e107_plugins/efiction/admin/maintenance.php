...
if($maint == "reviews") {
    ...
    e107::getMessage()->addInfo("Stats calculation started. For more info set debug mode on.") ;
	...
	if($check = e107::getDb()->retrieve($query)) {
        e107::getMessage()->addDebug($query);
    }
    else {
        $query = "INSERT INTO ".TABLEPREFIX."fanfiction_stats(`sitekey`) VALUES('".SITEKEY."')";
        e107::getDb()->gen($query);
        e107::getMessage()->addDebug($query);
    }
 
    $query = "SELECT seriesid FROM ".TABLEPREFIX."fanfiction_series"; 
	$serieslist = e107::getDb()->retrieve($query, true);
    e107::getMessage()->addDebug($query);
    ...
    $query = "SELECT COUNT(sid) as totals, SUM(wordcount) as totalwords FROM ".TABLEPREFIX."fanfiction_stories WHERE validated > 0 LIMIT 1 ";
    $storiesquery = e107::getDb()->retrieve($query);
    e107::getMessage()->addDebug($query);
    $stories = $storiesquery['totals'];
    $words = $storiesquery['totalwords'];    
    
	$query = "SELECT count(uid) FROM ".TABLEPREFIX."fanfiction_authorprefs WHERE stories > 0 LIMIT 1 ";
    $authors = e107::getDb()->retrieve($query);
    e107::getMessage()->addDebug($query);
    
    $query =  "UPDATE ".TABLEPREFIX."fanfiction_stats SET stories = '$stories', authors = '$authors', wordcount = '$words' WHERE sitekey = '".SITEKEY."'"; 
    e107::getDb()->gen($query);
    e107::getMessage()->addDebug($query);   
 
    $query = "SELECT COUNT(chapid) as chapters FROM ".TABLEPREFIX."fanfiction_chapters where validated > 0";
    $chapters= e107::getDb()->retrieve($query);
    e107::getMessage()->addDebug($query);
 
    $query = "SELECT COUNT("._UIDFIELD.") as totalm FROM "._AUTHORTABLE;
    $members = e107::getDb()->retrieve($query);
    e107::getMessage()->addDebug($query);    
   
    $query = "SELECT "._UIDFIELD." as uid FROM "._AUTHORTABLE." ORDER BY "._UIDFIELD." DESC LIMIT 1";
    $newest = e107::getDb()->retrieve($query);
    e107::getMessage()->addDebug($query); 
 
    /* REVIEWS STATS */
    $query = "SELECT COUNT(comment_id) as totalr FROM ".TABLEPREFIX."fanfiction_comments WHERE comment_type = 'ST' OR comment_type = 'SE' LIMIT 1";
	$totalr = e107::getDb()->retrieve($query);
    e107::getMessage()->addDebug($query);
	$reviews = $totalr;
    
    $query = "SELECT COUNT(comment_id) as totalc FROM ".TABLEPREFIX."fanfiction_comments WHERE comment_type = 'CH' LIMIT 1";
 	$totalc = e107::getDb()->retrieve($query);
    e107::getMessage()->addDebug($query);
	$comments = $totalc;   
 
    $query = "SELECT COUNT(DISTINCT comment_author_id) AS reviewers FROM ".MPREFIX."fanfiction_comments WHERE comment_type = 'ST' OR comment_type = 'SE' AND comment_author_id != 0 LIMIT 1";
	$reviewers = e107::getDb()->retrieve($query);
    e107::getMessage()->addDebug($query);
 
    $query = "SELECT COUNT(DISTINCT comment_author_id) AS commenters FROM ".MPREFIX."fanfiction_comments WHERE comment_type = 'CH' AND comment_author_id != 0  LIMIT 1";
 	$commenters = e107::getDb()->retrieve($query);
    e107::getMessage()->addDebug($query);
 
    $query = "UPDATE ".TABLEPREFIX."fanfiction_stats SET series = '$totalseries', chapters = '$chapters', members = '$members', newestmember = '$newest', 
    reviews = '$reviews', 
    reviewers = '$reviewers',
    comments = '$comments', 
    ...
    e107::getMessage()->addInfo("Stats calculation ended.") ;
